...
🎓 MSc Advanced Computer Science, Individual Project
...
```shell
git clone --depth=1 https://github.com/mfmsajidh/UoL-IndividualProject-NextJS my-project-name
cd my-project-name
npm install
```
...
Copy `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` and `CLERK_SECRET_KEY` generated by your [Clerk.com](https://clerk.com) account into `.env.local` file (not tracked by Git):
